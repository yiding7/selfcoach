...
- [苹果健康与有氧](#苹果健康与有氧)
- [命令一览](#命令一览)
- [**技术栈与数据流转**](#技术栈与数据流转) ← 你的数据和你加的资料走到哪、被谁读
- [目录结构与隐私](#目录结构与隐私)
- [常见问题](#常见问题)
...
换个模型历史就没法对照了。分工的具体边界见
[技术栈与数据流转](#技术栈与数据流转)。

**数据是你的，而且看得见摸得着。** 零第三方依赖、纯文本存储、整个代码库
只有两个地方出网（训记 API 和一个默认关闭的可选 LLM 适配器），没有遥测。
你自己放进来的知识库和病史资料**一律不进版本库、不出本机** ——
去哪了、谁读它，[技术栈与数据流转](#技术栈与数据流转)里逐条列了。
...
三个容易踩的：**哑铃按单只手的重量写**（解析器知道要乘二，写合计会让纵向对比失真）；
**只有第一行 `#` 是标题行**，之后的 `#` 都只当注释；中途降重量照实写，别凑整。

```bash
hc log --syntax                              # 完整语法
hc log --file examples/workout.txt --dry-run # 拿样例试一遍，不写入
```

### 体重、围度、饮食这些呢

也一样，跟助手说人话最省事（「今天腰围 93.5」「昨晚 3 杯啤酒」），
它会写进正确位置，**写之前先给你看摘要**。

想自己动手就照着 **[`examples/`](examples/README.md)** 里的样例写 ——
体重围度、苹果健康指标、饮食各一份，字段和取值都标了。
`tests/test_examples.py` 盯着它们，格式改了样例会跟着红。

> ⚠️ 样例**故意不放在 `data/` 里**：`store` 按 `data/**/*.jsonl` 通配读取，
> 样例文件拷进去会被当成真实数据算进趋势。照着格式写进你自己的文件。
...
这一节回答两个问题：**这东西是用什么搭的**，以及**我的数据和我自己加的资料，
到底走到哪、被谁读、会不会离开这台机器**。
...
| | 是什么 | 为什么 |
|---|---|---|
| 运行时 | **Python ≥ 3.11，零第三方依赖** | `pyproject.toml` 里 `dependencies = []`。clone 下来就能跑，不用 `pip install` 任何东西 |
| 存储 | **`.jsonl` 纯文本**（一行一条 JSON）+ `.json` 配置表 | 能用 `grep`、能用编辑器改、能进 git、二十年后还打得开。没有数据库 |
| 去重 | 每条带 `id`，**同 `id` 覆盖不插入** | 重复导入是安全的，同步可以随便重跑 |
| 分析 | 确定性 Python 函数，全部有单元测试（`uv run pytest`） | 数字归脚本。换模型不换结果 |
| 报告 | **自包含单文件 HTML**，CSS 和 SVG 全内联 | 没有 CDN、没有外部请求。断网能看，发给谁都能看 |
| 模型 | 由宿主提供（Claude Code / Codex / Cursor …） | 项目本身**不绑定任何模型厂商** |
| 后台 | macOS `launchd` / Linux `cron`，可选 | 没有常驻服务、没有端口、没有守护进程 |
| PDF | 可选 `brew install poppler` | 只影响「能不能读你自己放进来的 PDF」，核心功能一个都不受影响 |

**整个代码库只有两个地方出网**，可以自己核：

```bash
grep -rn "urlopen" src --include="*.py"
#   src/health_assistant/xunji/client.py   ← 训记 API，key 在 .env
#   src/health_assistant/llm.py            ← 可选 LLM 适配器，默认关闭
```

没有遥测、没有崩溃上报、没有使用统计。第二个出网点（`llm.py`）**只在你自己在
`.env` 里填了 `HC_LLM_*` 时才启用**，它是给 cron 无人值守出周报用的；在
Claude Code 里日常使用完全不碰它。而且即便启用，它**只发聚合后的 findings，
不发原始餐食、用药、体检数值**。
...
```
① 训记 App ──── hc sync ──────┐   （唯一的联网入口，需要 .env 里的 key）
② 苹果健康导出.zip ─ hc import-health ┤
③ 自己敲 ─── hc log / hc setup ┤──→  data/  （本机文件）
④ 跟助手说人话 ─ 助手转格式写入 ┘        「今天腰围 93.5」「昨晚 3 杯啤酒」
                                        写盘前一定先给你看摘要
```

**没有训记也是一等公民路径** —— 走 ③ ④，后续分析完全一样。
见 [没有训记怎么办](#没有训记怎么办) 和 [`examples/`](examples/README.md)。
...
```
                        ┌─────────────────────────────────────┐
   data/  ─────────────→│  确定性 Python 脚本                  │
   knowledge/movements/ │  吨位、估算 1RM、对比、处方、骰子     │──→ facts.json
   knowledge/training/  │  有单元测试盯着                      │     报告 HTML
   knowledge/nutrition/ └─────────────────────────────────────┘        │
                                                                       ↓
   knowledge/coach/       ┌──────────────────────────────┐      只负责措辞
   knowledge/measurement/→│  模型（宿主提供）             │←─────────────┘
   profile/               │  读散文、定语气、组织表达      │
   knowledge/library/     └──────────────────────────────┘
   你自己的知识库              **不参与算数**
```

**模型只能使用 `facts.json` 里出现过的数字。** 想要新的分析角度，它得去跑
`hc` 命令，不能心算。这不是约定俗成，`llm.py` 里有一道数字幻觉拦截：
叙述里出现的每个数字都必须能在 facts 里找到，找不到就丢弃整段叙述、
退回纯数据模式。**报告宁可少一段话，也不能出现一个编造的体重。**

### 你自己加的东西，去哪了

这是最容易担心的部分，所以逐条列清。**下面全部不进版本库、不出本机**：

| 你加的东西 | 放哪 | 谁读它 | 进 git 吗 |
|---|---|---|---|
| 一整套现成的知识库（课程包、资料库） | `knowledge/<随便起名>/` 保持原样 | **模型**，按索引定位 | ❌ 白名单挡掉 |
| 零散单份资料（论文、讲义、笔记） | `knowledge/library/<主题>/` | **模型**，按 `INDEX.md` 定位 | ❌ 整个目录忽略 |
| 病史、用药、伤病、忌口、沟通偏好 | `profile/` | **模型** | ❌ |
...
- **`knowledge/` 根目录是 gitignore 白名单**，只放行五个区 + `library/` +
  `README.md`。所以整套资料丢在根上，**放错位置的后果是「被忽略」而不是「被公开」** ——
  这是有意选的方向
- **模型不会通读你的知识库。** 它先读 `knowledge/library/INDEX.md`
  或那套资料自带的索引，定位到两三份再打开。几百份 PDF 全读会挤爆上下文，
  也会让真正要紧的东西被淹掉。没有索引就让它生成一份：
  「查找 `<路径>` 的文件，写一份索引」

### 原始数据永不改写

几个地方是**只追加、不修改**的，这不是洁癖，是为了让「当时怎么想的、后来为什么改」
半年后还看得见：
...
- **带走**：`data/` `profile/` 就是全部，拷走即可。都是纯文本，不依赖这个工具也读得懂
- **跨设备同步**：在 `data/` 里单独 `git init` 一个**私有**仓库
- **删掉**：`rm -rf data/ profile/`。没有云端副本，没有别处的缓存
- **看看到底存了些什么**：`hc doctor` 会列出每一类数据的位置和条数
...
> **数据从哪进来、被谁读、会不会出本机、怎么带走或删干净 ——
> 见上一节 [技术栈与数据流转](#技术栈与数据流转)**，那里有完整的流向图，
> 以及「你自己加的知识库和资料去哪了」的逐条说明。
