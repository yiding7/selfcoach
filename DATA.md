...
```json
{"schema":"ha.body/1","id":"apple:weight:2026-08-04","source":"apple_health","date":"2026-08-04","type":"weight","value":80.4,"unit":"kg","label":"体重"}
```
...
{"schema":"ha.meal/1","id":"manual:2026-08-04:lunch:1","source":"manual","date":"2026-08-04",
 ...
 "protein_g":31,"fat_g":3.6,"carb_g":0,"confidence":"estimated"}
```
...
| **C. 直接跟助手说** | 「今天腰围 93.5」 | 助手写进 `data/body/YYYY.jsonl` |
...
**唯一值得补记的是 RPE**。没有 RPE 时，配重建议只能靠次数反推，精度差不少。
每个动作最后一组记一个就够，不用每组都记。
...
| 你说 | 助手做什么 |
|---|---|
| 「今天腰围 93.5，晨起空腹量的」 | 写 `data/body/`，`type: weist`，`source: manual` |
