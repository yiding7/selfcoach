...
FACTS = {
    ...
    "findings": {"优点": [{"text": "胸的估算 1RM 从 53.3kg 提升到 60.0kg（+12.5%）",
                           "metrics": {"before": 53.3, "after": 60.0, "pct": 12.5}}],
                 "缺点": [], "改进点": [], "信息": []},
    "body": {"change_kg": -0.4, "rate_pct_per_week": -0.35, "latest_trend_kg": 79.9},
}
...
class TestNumberGuard:
    ...
    def test_rejects_invented_number(self):
        ...
        bad = check_numbers("你的深蹲已经到 137.5 kg 了", FACTS)
        assert "137.5" in bad, "编造的数字必须被拦下"

    def test_rejects_invented_percentage(self):
        assert "47.3" in check_numbers("容量涨了 47.3%", FACTS)
...
class TestNarrateResilience:
    ...
    def test_slot_with_hallucinated_number_is_dropped(self, monkeypatch):
        ...
        monkeypatch.setattr(llm, "complete", lambda *a, **k: (
            '{"opening": "这周练了 3 次，很稳。",'
            ' "training": "你的卧推已经到 137.5kg 了，非常了不起。"}'))
