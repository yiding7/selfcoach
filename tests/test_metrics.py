...
class TestRollingWeight:
    def test_damps_noise(self):
        """真实 7 月数据里 7/16 的 81.6kg 是离群点，移动均线必须把它压下去。"""
        records = [{"type": "weight", "date": d, "value": v} for d, v in [
            ("2026-07-12", 79.8), ("2026-07-13", 79.9), ("2026-07-14", 79.8),
            ("2026-07-16", 81.6), ("2026-07-18", 80.0), ("2026-07-19", 79.9)]]
        trend = rolling_weight(records, window=7)
        smoothed = dict(trend)["2026-07-16"]
        assert 79.8 < smoothed < 80.4, "离群点应当被显著压制"
