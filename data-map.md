...
hc calib set '面拉' --gym 艾克仕 --ratio 0.5 --note '实测手行程 = 配重片 2 倍'

# 加法：滑车自重 / 配重 / 助力。**可以是负数**
hc calib set '腿举' --gym BA --offset 53 --note '机身标 118lbs/53kg'
hc calib set '引体向上' --gym BA --offset -15 --note '套助力带减 15kg'
hc calib set '引体向上' --gym BA --offset 10 --note '腰间挂 10kg 配重'
...
| 馆 | 动作 | 变换 | 依据 |
|---|---|---|---|
| BA | 直杆绳索下压 / 绳索臂屈伸 / 面拉 | `×1` | 实测 1:1（手行程 = 配重片行程） |
| BA | 腿举 | `+53` | 机身标 118 lbs |
| BA | 哈克机深蹲 | `+57` | 机身标 |
| 艾克仕 | 直杆绳索下压 / 绳索臂屈伸 / 面拉 | `×0.5` | 实测 2:1 |

**记录一律是配重片读数**（自由重量记杆+片），折算全交给上面这张表。

还没登记的：

- 艾克仕的腿举 / 哈克机深蹲 —— 机身有没有印 start 还没看。
  **但那两个动作的历史记录一直是纯片重，口径是一致的**，所以现在不写规则
  也不会算错，只是那个数还没加上滑车自重。看到了补一条就行
- 其余馆的绳索：下次去顺手数一下行程（手移动距离 ÷ 配重片上升距离，30 秒）
...
hc gym set 2026-08-20 BA                 # 标一天（填「空」= 撤销）
