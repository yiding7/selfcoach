...
**记录口径：照抄配重片上的数字，折算交给脚本**（2026-08-25 起）。
早先的口径是「记手上的力 = 标称重量 ÷ 传动比」，也就是人在现场心算 ——
现在不这么做了：人记原始读数，传动比一台机器登记一次，`hc calib` 在读取时折算。
测传动比最省事的办法是数行程（手移动的距离 ÷ 配重片上升的距离）。
...
> 换馆已经解释掉的跳变不会再报。两边都标了场地、场地不同、而这个动作的负荷
> 本来就不跨馆成立（器械/绳索/史密斯…）—— 那么「数字变了」是必然的，不是发现。
> 实测：加这一条之后 6 条命中降到 1 条，剩的那条是同一个馆里的真跳变。

**三种处置，你选一个：**

```bash
# 1 改原始记录数据（去训记改那天，然后重抓那一天）
hc sync train --since 2026-07-16 --until 2026-07-16 --force

# 2 这台机器就是和别台不一样 —— 写一条挂在「馆 + 动作」上的折算规则，原始文件不动
hc calib set '面拉' --gym 甲馆 --ratio 0.5 --note '实测 2:1'
...
##### 前提：记录口径先统一（2026-08-25 用户定死）

折算只在「记录本身有一条固定规矩」时才成立。那条规矩是：

| 器材 | 怎么记 |
|---|---|
| 自由重量（杠铃、哑铃、壶铃） | **杆 + 片** |
| 器械（哈克、腿举、绳索、悍马、史密斯…） | **只记片重** |

于是每台机器和「真实的力」之间只差一个固定的变换，而那个变换是机器的属性 ——
可以一次登记、永久生效。口径不统一的话，同一台机器上今天记 107、明天记 50，
再好的规则也救不回来。

##### 两种变换，就这两种

```bash
# 乘法：滑轮组传动比。2:1 就填 0.5
hc calib set '面拉' --gym 甲馆 --ratio 0.5 --note '实测手行程 = 配重片 2 倍'

# 加法：滑车自重 / 配重 / 助力。**可以是负数**
hc calib set '腿举' --gym 甲馆 --offset 50 --note '机身标的滑车自重'
hc calib set '引体向上' --gym 甲馆 --offset -15 --note '套助力带减 15kg'
hc calib set '引体向上' --gym 甲馆 --offset 10 --note '腰间挂 10kg 配重'

hc calib list                       # 看所有规则，@馆名 就是它的作用域
```

| | 乘法 `--ratio` | 加法 `--offset` |
|---|---|---|
| 变换 | `实际 = 记录 × ratio` | `实际 = 记录 + offset` |
| 用在哪 | 绳索 / 龙门（滑轮组传动比） | 哈克 / 腿举 / 倒蹬的滑车自重；引体的配重与助力 |
| 正负 | 只能是正数 | **正负都行**：配重为正，助力为负 |

**加法换算不出乘法**：同一天各组片重不同时（腿举某次是 40/70/70/70），
一个系数对不上任何一组。反过来也一样。所以两种都要有，不能只留一个然后
「大概换算一下」。

两者同时命中同一个动作时**先乘后加**（传动比作用在配重片读数上，滑车自重
是读数之外另加的一块）。现实里几乎不会同时发生，但顺序定死了，
同一份数据两次运行必然得出同一个结果。

> **助力带和配重不要拆成两个动作名。** `引体向上（助力带）` 能解决对比问题，
> 但它把同一个动作拆成两条互不相干的曲线，纵向进步就永远看不出来了 ——
> 用户 2026-08-10 明确否掉过这个方案。一正一负两条 offset 才是对的做法。

##### 作用域：挂在**馆 + 动作**上，没有日期（2026-08-25 起是强制的）

**传动比和滑车自重是那台机器的物理属性，不是某一天的属性。** 某个馆的龙门
今天 2:1、明天还是 2:1。用日期区间去表达「在哪台机器上练的」，每换一次馆
就要补一条规则，**补漏一条就静默错一次**。挂在馆上是一次定义、永久生效。

**四条纪律：**

- **变换类规则必须有 `--gym`，且不能带日期**，CLI 会拒绝。`--confirm` 反过来：
  必须有 `--date`，不能有 `--gym`（它确认的是某一次，不是某台机器）
- **没标场地的记录永远不会被按馆的规则命中。** `gym` 是 `None` 时不匹配任何
  规则 —— 「不知道在哪练的」绝不能当成「就是那个馆」，那是最难查的一种错
- **`--ratio 1.0` 是有意义的**，虽然它不改任何数字。它把「测过了，就是 1:1」
  和「还没人测过」区分开 —— 后者不该被静默当成前者
- **场地必须先于折算挂上**（`store.load_sessions()` 里的顺序），
  反过来会让每一条规则永远匹配不到，而且没有任何报错

##### `ignore` 已退役（2026-08-25）

原来还有第四个处置「这次该动作不参与对比」。它存在的两条理由 —— 腿举和哈克
历史上「只记片重 vs 记片重+start」—— 在记录口径统一之后都变成了一条 `offset`。
而 `ignore` 是这张表里唯一会**藏数据**的动作，能不留就不留。

文件里的旧 `ignore` 行**不删**（只追加），但它们不再生效。`hc calib list`
会把它们单独列出来说明 —— 一条写在文件里、看起来生效、实际不生效的规则，
是这个项目最不能接受的那种错。要真正推翻就新写一条 `--supersedes <旧ID>`。

> `--supersedes` 接受**逗号分隔的多个 ID**：一条新的 offset 规则同时取代
> 「旧的 ignore 起算线」和「更早那版 offset」是常有的事。

##### 已登记的规则

**真相源是 `hc calib list`**，不是这份文档 —— 写在这里的表格会在下一次加规则时
过期，而过期的文档比没有文档更容易骗人。另外馆名是使用者的行踪信息，
这个仓库是公开的，所以具体登记了哪几个馆、哪几台机器只存在两个地方：
`data/load-calibration.jsonl`（gitignore）和 `profile/personal-context.md`（不进版本库）。

```bash
hc calib list        # 现在生效的全部规则，含各自的 note 和失效的旧规则
hc gym               # 标了哪几个馆、覆盖率多少
```

**记录一律是配重片读数**（自由重量记杆+片），折算全交给规则表。

**没登记 ≠ 算错。** 一台还没量过 start 的哈克，只要那个动作的历史记录口径一致
（一直是纯片重），不写规则也不会算错 —— 只是那个数还没加上滑车自重，
偏小但偏得一致，纵向对比照样成立。所以补规则不急，**改口径才急**。

下次去新馆顺手做两件事：绳索数一下行程（手移动距离 ÷ 配重片上升距离，30 秒），
哈克/腿举看一眼机身有没有印 start。

> **按馆的规则会回溯到那个馆的全部历史**，因为它没有日期边界 ——
> 这正是「挂在馆上」的意思。写之前先想清楚：一条 `--offset 53` 会把那个馆
> 此前的全部记录一起加 53。前提是那些记录都已经是新口径（只记片重）；
...
换了台机器。场地是那个能**预测**机器变化的变量 —— 标注量是 `训练次数 × 1`，
而手写 calib 规则的工作量是 `动作数 × 换馆次数`。

```bash
hc gym                                   # 覆盖率：标了多少天、有哪几个馆
hc gym set 2026-08-20 甲馆               # 标一天（填「空」= 撤销）
hc gym export --out data/gyms.tsv        # 导出待填表格
hc gym import data/gyms.tsv              # 填完导回来
```

> **场地表和口径表是两个文件，别去合它们。** 一度合过（2026-08-24），
> 当天就回滚了：两种行的字段几乎不重叠，塞进一个 jsonl 之后每一行都要先看
> `kind` 才知道该读哪些字段，格式的自描述性反而变差了。它们相关，但不同构。

**标了之后 `hc compare` 的行为**（用户 2026-08-21 拍板的那一档）：

| | 换馆时 |
|---|---|
| 杠铃、哑铃、自重 | **照常比** —— 64 kg 在哪个馆都是 64 kg |
| 器械、绳索、史密斯、哈克、腿举 | 顶组和估算 1RM 两端置空，**组数和容量照常计入** |

判据表在 `knowledge/movements/site-dependence.json`，改表不用改代码。
认不出器械类型的动作保守当作不可比，并且会在 `hc gym` 里列出来。

三条纪律：

- **空是答案，不是缺失。** 没标场地的那天 `gym` 是 `None`，对比行为和加这个
  功能之前**完全一样**。`None` 是「不知道」，不是「同一个馆」—— 只有两边都标了
  且不同才算换馆
- **绝不猜。** 「工作日一个馆、周末一个馆」这类规律用户明说过不一定准，
  不许拿它自动填任何一天
- **不要写进训记的心得。** 训记没有独立的热量字段，kcal 就塞在心得里
  （`"calorie:228"`）—— 往心得写一个字，那次的热量就没了。
  2026-08-16/18/20/21 四次写了馆名，四次都丢了 kcal

> 导出的表里是**真实训练日期和动作名**，放 `data/` 下面（整个目录已 gitignore）。
> 写到别处时 `hc gym export` 会警告。
