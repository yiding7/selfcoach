...
def normalize_group(raw: str | None) -> str | None:
    ...
    alias = {
        ...
        "腹肌": "腹部", "核心": "腹部", "腹": "腹部", "核心稳定": "腹部",
        "股四头": "腿", "腘绳": "腿", "大腿": "腿", "腿部": "腿",
        "臀部": "臀", "肩部": "肩", "背部": "背", "胸部": "胸",
        "有氧运动": "有氧", "心肺": "有氧",
    }
    s = alias.get(s, s)
    # 训记的 type 里混着动作**格式**标签（计时动作、功能性…），那不是肌群。
    # 早先原样收下，结果它们各自变成一个伪肌群，还盖掉了本地分类表 ——
    # 「平板支撑_计时」被判成「计时动作」，跟同一个动作的「平板支撑」不可比。
    # 认不出来就交回给 taxonomy / 规则去判，别硬当肌群用。
    return s if s in _valid_groups() else None
